...
namespace SummonManager
{
    public partial class ShowSummonWare : Form
    {
        ...
        private void bAdd_Click(object sender, EventArgs e)//сохранить и передать в отдел логистики
        {
            /* if (tbQUANTITY.Value == 0)
             {
                 MessageBox.Show("Введите количество изделий!");
             ...
             if (MessageBox.Show("Вы действительно хотите сохранить и пометить извещение как готовое к отгрузке?", "Внимание!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
             {
                 return;
             }
             DBSummon dbs = new DBSummon();
             ...
             dbs.PassToManagerFinish(SVO,UVO.id);
             //MessageBox.Show("Извещение успешно передано в коммерческий отдел!");
             this.Close();*/
        }

        private void ShowSummonWare_FormClosed(object sender, FormClosedEventArgs e)
        {
